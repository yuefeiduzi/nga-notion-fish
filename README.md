...
- **无图模式**：默认不加载图片（摸鱼 + 省流量），正文里显示「图片」占位按钮，点一下即可全屏查看。
- **图片全屏预览**：正文里的图最大 70vh 高（长图不会占满一屏），点击进全屏：滚轮/± 缩放、拖动平移、
...
- **接管而不是换皮**：站内跳转是正常整页导航（NGA 的正文由它自己的 JS 注入，抓 HTML 拿不到），
  但原站从 `document_start` 起就被藏住，所以你不会看到 NGA 的界面闪一下。
